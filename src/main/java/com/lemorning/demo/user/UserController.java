...
import com.lemorning.demo.config.JwtTokenUtil;
...
import com.lemorning.demo.point.Point;
import com.lemorning.demo.point.PointChargeDto;
import com.lemorning.demo.point.PointService;
import com.lemorning.demo.upload.S3Service;
import com.lemorning.demo.user.response.ProfileResDto;
import lombok.*;
...
public class UserController {
    ...
    public Response signup(@RequestBody UserDto infoDto) { // 회원 추가
        Response response = new Response();
        try {
            User savedUser = userService.save(infoDto);
            if(savedUser==null){
                throw new Exception();
            }
            response.setResponse("success");
